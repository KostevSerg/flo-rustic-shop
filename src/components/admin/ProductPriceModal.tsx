...
const ProductPriceModal = ({ product, cities, onSetPrice, onClose }: ProductPriceModalProps) => {
  return (
    <>
      <div
      ...
      <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[101] w-[90vw] max-w-3xl bg-card border border-border rounded-xl shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-border">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-bold">Цены для "{product.name}"</h3>
            <button
              onClick={onClose}
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Базовая цена: {product.base_price} ₽
          </p>
        </div>
